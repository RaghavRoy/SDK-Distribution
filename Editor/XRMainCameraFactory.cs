...
namespace JioXSDK
{
    public static class XRMainCameraFactory
    {
        ...
        static Camera CreateVRMainCamera(CreateUtils.InputType inputType)
        {
            ...
            var additionalCamera = CreateAdditionalCamera();
            additionalCamera.nearClipPlane = 0.3f;

            additionalCamera.transform.SetParent(camera.transform);
        ...
        static Camera CreateAdditionalCamera()
        {
            var cameraGo = ObjectFactory.CreateGameObject(
                "Camera",
                typeof(Camera),
